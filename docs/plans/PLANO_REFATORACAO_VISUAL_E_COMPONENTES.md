...
| **A** | Auditoria: lixo & segurança (pré-refatoração) | ✅ **Concluída** | 12/08/2026 | tsc limpo · **443 testes OK** · build OK · guardrails verde · **19 órfãos + 4 serviços IA removidos** · `tmp/` fora do tracking · `@testing-library/dom` removida |
| **B** | Auditoria: DRY de utilitários | ✅ **Concluída** | 12/08/2026 | tsc limpo · **443 testes OK** · build OK · guardrails verde · `todayISO()` (17 migrações) + `roundToDecimals()` (23 migrações) · CurrencyInput usa `formatCurrency` central |
| **C** | Auditoria: primitivos Eyebrow/GlassCard | ✅ **Concluída** | 12/08/2026 | tsc limpo · **443 testes OK** · build OK · guardrails verde · 17 Eyebrows + 4 GlassCards migrados · imports de cards DRY |
| **D** | Auditoria: decomposição de monolitos | ✅ **Concluída** | 12/08/2026 | tsc limpo · **443 testes OK** · build OK · guardrails verde · **CSV Panel 1193→669** e **FloatingCalculator 1127→407** · prop morta `paymentItems` removida da cadeia |

### Auditoria de Limpeza (A→D) — Registro de mudanças
- **Fase A — Lixo & segurança:** removidos 19 arquivos órfãos confirmados por varredura de imports (0 referências em `src/`, inclusive testes) + 4 serviços de IA desativados da UI (`geminiService`, `naturalLanguageParser`, `aiIcons`, `fundamentalsService` — ~500 linhas); `tmp/` (24 scripts de debug) fora do tracking (auditoria movida para `scripts/audit/`); `@testing-library/dom` removida (era transitiva do `@testing-library/react`).
- **Fase B — DRY de utilitários:** criado `todayISO()` em `format.ts` (17× `format(new Date(),'yyyy-MM-dd')` consolidados) e migradas 23× `Number(x.toFixed(n))` → `roundToDecimals(x, n)` (script `scripts/audit/migrate_round_to.mjs` com parser de parênteses); `CurrencyInput` passou a usar `formatCurrency` central (removeu `BRL_FORMATTER` próprio).
- **Fase C — Primitivos:** criados `ui/eyebrow.tsx` (labels uppercase com tone/weight/tracking) e `ui/glass-card.tsx`; migrados 17 Eyebrows e 4 GlassCards do `MobileMenuSheet` (área de clique preservada).
- **Fase D — Monolitos:** `CreditCardCsvReconciliationPanel` **1193→669** (wizard extraído em 9 componentes `Csv*Step`/`CsvWizardStepper`/`CsvAlertBanner`/`CsvStepFooter` + `utils/csvReconciliationUi.ts` com tipos/helpers); `FloatingCalculator` **1127→407** (hooks `useCalculatorExpression`, `useCalculatorTargetInput`, `useCalculatorIconDrag` + componentes `CalculatorPanel`/`CalculatorKeypad`); prop morta `paymentItems` removida da cadeia `ContasModals`/`Contas`; cards de conciliação agora importam `MissingDraft`/`ConflictDraft` de `csvReconciliationUi` (fim da duplicação de tipos).
...
| **v2.1** | **12/08/2026** | **Auditoria de limpeza pré-refatoração executada (A→D):** 23 arquivos removidos (19 órfãos + 4 serviços IA), DRY de `todayISO`/`roundToDecimals`/`formatCurrency`, primitivos `Eyebrow`/`GlassCard`, e decomposição de `CreditCardCsvReconciliationPanel` (1193→669) e `FloatingCalculator` (1127→407). 443 testes em verde em todas as fases. |
