...
| **R7+** | **Varredura de Harmonia Completa** | Todos os `pages/*` e `components/*` (auditoria real: ~60 arquivos com `formatCurrency` direto, 126 labels inline, cards sem glass) | Regras: (1) `formatCurrency` em exibição → `AmountText` (tone/weight via props; string-presas como tooltip continuam `format.ts`); (2) cards → `surface-glass border-glass`; (3) labels `uppercase tracking-*` estruturais → `Eyebrow`; (4) banners → tokens únicos `danger`/`success`; (5) `AuthShell` DRY para as 5 telas de auth/onboarding | 0 mudança de contrato; guardrails verde; blocos validados com tsc/lint/testes/build + commit por bloco |
...
| **R5** | ✅ Concluída | Itens 5–6 da seção 4.4: **ContasStats** migrado de `formatCurrency` para **`AmountText`** (4 KPIs: Faturas em Aberto, a Pagar, a Receber, Saldo Pendente com tone por sinal; size md — evita overflow no grid 2-col mobile). Novo **`utils/debtStatus.ts`** (DRY): `getDebtDueStatus` (Quitada/Atrasada/Vence Hoje/Vence em Breve/A Vencer por `due_date` + status, diff via Date.UTC imune a DST) + 7 testes; `DebtsSection` usa o chip de status derivado (substitui Pago/Pendente). Verificados sem mudanças: bankBranding já no `CreditCardSection`, `CreditCardTimeline` refinada (badges/progresso), wizard CSV já refinado na auditoria — testes de conciliação verdes. **477 testes verdes**, lint zero, guardrails OK, build OK. |
| **R6** | ✅ Concluída | Itens 4.5.3 e 4.6.3: **`CategoryFormModal`** ganhou catálogo Lucide **pesquisável** (Input `type=search` com `aria-label`, filtra os 23 ícones case-insensitive, mantém o selecionado visível, mensagem "Nenhum ícone encontrado", Enter não submete o form — achados do review aplicados). **Charts de relatório** com linhas finas: `AnnualFlowChart` (3 séries) e `CumulativeBalanceChart` com stroke 2.5 → 2 (padronizado com `CategoryTrendChart`; tracejadas de comparação seguem 1.5). Verificados sem mudanças: `ReportsTabButton`/tabs de período, `ChartSeriesBadges` (toggle 1-toque), grid de categorias com barra de limite, Settings (temas/biometria/admin). **477 testes verdes**, lint zero, guardrails OK, build OK. |
| R7 | ⏳ Pendente | — |
| **R7+** | 🔄 Em andamento | **Bloco A ✅** (12/08): `AuthShell` DRY nas 5 telas de auth/onboarding (Login/Register/ForgotPassword/ResetPassword/OnboardingCategories — ~15 linhas duplicadas por página eliminadas) + cards `surface-glass border-glass` + banners unificados nos tokens danger/success + `PortfolioKpiBar` → `AmountText`. **Bloco B ✅**: varredura Dashboard details (FinancialHealthDetail, SubscriptionsDetail, CategoryBreakdownDetail, LimitsOverviewDetail, RecurringExpenseDetailModal, DashboardCategoryDetailModal) + Reports (MonthlyReportView, AnnualReportView, ReportPendingDebtsWidget, ReportUnifiedCompositionCard, ReportsCategoryRowButton, CategoryDetailModal) — `formatCurrency` → `AmountText` (tone semântico, forceSign nas variações; strings/tooltips/SVG `<text>` mantêm `format.ts`); `as any` nos dados de charts → cast `Parameters<typeof Chart>[0]['data']`. **Bloco C ✅** (12/08): varredura em ~27 arquivos — Categorias (Expense/IncomeCategoryGrid), CreditCards (BillExpenseRowButton, ComparisonRowCard, ConflictDraftCard, CreditCardSection, CreditCardTimeline, CsvReviewStep, MissingDraftCard, SuspiciousDraftCard), Debts (DebtActionConfirmModals, DebtsSection), Investimentos (AssetAnalyticsCard, AssetDetailModal, AssetYieldConfigCard, B3AdjustmentCard, EvolutionChart, HoldingsTable, InvestmentConflictCard, LedgerBook, MonthlyActivityCard, PortfolioPieChart, QuickBalanceUpdateModal, SmartAporteSimulator, SuspiciousInvestmentCard) e Transactions (TransactionDetailDrawer, TransactionFormModal). Regra de cores fiel: tones/className preservam a cor original; strings (tickFormatter do recharts, textos de insights) mantêm `format.ts`. R2 ⏳ (passe fino do shell) |
