...
| — | **Redesign R12 — Polimento final obsidian (KPIs/Reports/Auth)** | `KpiCard`: borda refratária `glass-refract` (substitui shadow-sm) + título → `Eyebrow tracking=widest` — herdado por Reports, ContasStats e PortfolioKpiBar. KPI panels de `MonthlyReportView`/`AnnualReportView` com glass-refract. Marca circular do `AuthShell` em vidro (surface-glass + ring-glass). 480 testes verdes, lint zero, guardrails OK, build OK | Redesign | ✅ |
| — | **Redesign R11 — Dashboard Premium (Hero & Widgets)** | `DashboardHero`: glass-refract + glow ambiente obsidian (radial color-mix), label → Eyebrow, sparkline com pulso fluorescente. `WidgetCard`: borda refratária no lugar de shadow-sm, ícone do widget em badge de vidro com borda, hover border-glass-strong. 480 testes verdes, lint zero, guardrails OK, build OK | Redesign | ✅ |
| — | **Redesign R10 — Cartões Apple Wallet 3D (Contas & Faturas)** | Novo `creditCards/CreditCardFace.tsx`: face do cartão com cor do banco (`resolveCardColor`) + `BANK_CARD_SCRIM` (contraste WCAG) + brilho refratário, chip holográfico e selo de bandeira (`ui/card-hologram.tsx` — SVG puro, allowlist consciente no guardrails como arte/marca), Fatura Atual com `AmountText` e barra de limite térmica (esmeralda→âmbar→coral em 60/80%). Tilt 3D no hover desktop (`perspective: 1000px`, máx ±6°, gate `(hover: hover)` para não conflitar com gestos mobile). Integrado no topo do conteúdo expandido do `CreditCardSection`. 480 testes verdes, lint zero, guardrails OK, build OK | Redesign | ✅ |
| — | **Redesign R9 — Feed Tátil Premium (Despesas & Rendas)** | `TransactionCard` (mobile + desktop): ícone de categoria em círculo de vidro com glow na cor da categoria (`color-mix` dinâmico), pills de parcela/competência em vidro no mobile (antes só desktop), valores com `AmountText` (incl. original tachado), micro-press com `TactilePress` (spring 450/25) no card inteiro; barra lateral de cor substituída pelo badge de ícone no mobile. Labels de seção "Parceladas"/"Despesas do mês" → `Eyebrow` (Expenses). 480 testes verdes, lint zero, guardrails OK, build OK | Redesign | ✅ |
| — | **Redesign R8 — Fundação Obsidian Premium & Micro-Interações** | Tokens `--glass-refract`/`--glass-refract-strong` (borda refratária metálica theme-aware via `color-mix`) + utilities `.glass-refract(/-strong)` aplicadas em `.glass-card-interactive` (hover refract+panel), KPI cards de Categorias e accordion de cartões. Primitivo `ui/tactile.tsx` (`TactilePress` — framer-motion spring 450/25, scale 0.985 tap / 1.01 hover, respeita reduced-motion, 3 testes) aplicado no accordion de cartões e tiles de criação de categoria. Pulso fluorescente do `ComparisonSparkline` validado + `pointer-events-none`. 480 testes verdes, lint zero, guardrails OK, build OK | Redesign | ✅ |
| — | **Redesign R7+ Bloco C — varredura de harmonia completa (Categorias/CreditCards/Debts/Investimentos/Transactions)** | ~27 arquivos migrados de `formatCurrency` → `AmountText` com regras fixas: sizes padronizados (md para KPIs text-base sm:text-lg, sm para text-sm, xs para valores pequenos), weights mapeados (font-black → extrabold), tones semânticos fiéis à cor original (income/expense/default/muted + className text-accent/text-warning/text-current p/ herança de cor), `forceSign` no fluxo líquido (MonthlyActivityCard). Strings preservadas em `format.ts`: tickFormatter do recharts (AssetDetailModal) e textos de insights (SmartAporteSimulator). Verificação por script: zero `AmountText` em contexto string; semântica tone conferida. 477 testes verdes, lint zero, guardrails OK, build OK | Redesign | ✅ |
| — | **Redesign R7+ Bloco B — Dashboard details & Reports com AmountText** | Varredura de harmonia: `FinancialHealthDetail`, `SubscriptionsDetail`, `CategoryBreakdownDetail`, `LimitsOverviewDetail`, `RecurringExpenseDetailModal`, `DashboardCategoryDetailModal`, `MonthlyReportView`, `AnnualReportView`, `ReportPendingDebtsWidget`, `ReportUnifiedCompositionCard`, `ReportsCategoryRowButton` e `CategoryDetailModal` — `formatCurrency` em exibição → `AmountText` (tone semântico por sinal, `forceSign` nas variações; strings/tooltips/SVG `<text>` seguem `format.ts`). `as any` nos dados dos charts substituído por cast tipado `Parameters<typeof Chart>[0]['data']` (via `unknown` onde os shapes não sobrepõem). 477 testes verdes, lint zero, guardrails OK, build OK | Redesign | ✅ |
| — | **Redesign R7+ Bloco A — AuthShell DRY + PortfolioKpiBar** | Novo `components/auth/AuthShell.tsx` (scaffold compartilhado: fundo centralizado + glow + marca vetorial + título) aplicado em Login/Register/ForgotPassword/ResetPassword/OnboardingCategories — elimina ~15 linhas duplicadas por página; cards de auth com `surface-glass border-glass`; banners de erro/sucesso unificados nos tokens danger/success; `PortfolioKpiBar` (Patrimônio/Proventos/Resultado Líquido) migrado para `AmountText` com tone + forceSign | Redesign | ✅ |
| — | **Redesign R6 — Relatórios & Categorias (catálogo pesquisável + linhas finas)** | `CategoryFormModal` com catálogo Lucide pesquisável (Input `type=search` + `aria-label`, filtro case-insensitive, ícone selecionado sempre visível, Enter sem submit). Charts com linhas finas: `AnnualFlowChart` (Rendas/Despesas/Investimentos) e `CumulativeBalanceChart` com stroke 2.5→2 (padrão já usado no `CategoryTrendChart`); comparações tracejadas mantidas em 1.5. `useReportsData`/`useAppSettings` intactos | Redesign | ✅ |
| — | **Redesign R5 — Contas & Faturas (KPIs + status de dívidas)** | `ContasStats` migrado para `AmountText` (4 KPIs com tone semântico; size md p/ grid mobile). Novo `utils/debtStatus.ts` (`getDebtDueStatus` + `DEBT_DUE_STATUS_LABEL`, diff por dias de calendário via `Date.UTC` — imune a DST) com 7 testes; `DebtsSection` exibe chip de status derivado (A Vencer/Vence Hoje/Atrasada/Quitada) no lugar do pill Pago/Pendente. Verificados sem mudanças: bankBranding no `CreditCardSection`, `CreditCardTimeline` (badges + progresso), wizard CSV (9 componentes da auditoria) — testes de conciliação verdes | Redesign | ✅ |
