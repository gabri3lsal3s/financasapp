...
import { addMonths, getCurrentMonthString, todayISO } from "@/utils/format";
...
export function NotificationsProvider({
  ...
  const [cardBillAlerts, setCardBillAlerts] = useState<
    Array<{
      cardName: string;
  ...
  const loadCardBillAlerts = useCallback(async () => {
    try {
      ...
      const cycleMap = (cyclesData || []).reduce<
        Record<string, { closing_day: number; due_day: number }>
      >((acc, row) => {
      ...
      for (const card of creditCards) {
        ...
        for (const comp of competences) {
          ...
          const cardExpensesFiltered = (cardExpenses || []).filter((exp) => {
            ...
            const expComp = resolveBillCompetence(exp.date, (c) =>
              resolveClosingDayLocal(card.id, c),
            );
            return expComp === comp;
          });
          ...
          const cardPaymentsFiltered = (cardPayments || []).filter((pay) => {
            ...
            const payComp = resolveBillCompetence(pay.payment_date, (c) =>
              resolveClosingDayLocal(card.id, c),
            );
            return payComp === comp;
          });

          const totalPago = cardPaymentsFiltered.reduce(
  ...
  const combinedAlerts = useMemo(() => {
    ...
    debtAlerts.forEach((alert) => {
      const prefix =
        alert.debtType === "payable" ? "Conta a pagar" : "Cobrança a receber";
      alertsList.push({
        id: `debt-${alert.id}`,
