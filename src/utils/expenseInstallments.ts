...
export function buildInstallmentDates(
  ...
  const normalizedTotal = Math.max(
    1,
    Math.min(60, Math.trunc(installmentTotal || 1)),
  );
 ...
 */
export async function fetchCardClosingDayContext(
  creditCardId: string,
  competences: string[],
) {
  ...
  if (!cardData || !Number.isFinite(cardData.closing_day)) {
    throw new Error(
      "Não foi possível calcular a competência da fatura para este cartão.",
    );
  }
  ...
  if (uniqueCompetences.length) {
    ...
    monthlyOverrides = (cycleRows || []) as Array<{
      competence: string;
      closing_day: number;
    }>;
  }

  const closingDayByCompetence = monthlyOverrides.reduce<
...
export function generateInstallmentPayloads(
  ...
  const installmentTotal = Math.max(
    1,
    Math.min(60, Math.trunc(installments || 1)),
  );
  const baseDate = new Date(`${expense.date || todayISO()}T00:00:00`);

  if (installmentTotal <= 1) {
    const singleExpenseDate = expense.date || todayISO();
    const billCompetence =
      expense.bill_competence ||
      (expense.payment_method === "credit_card" &&
      expense.credit_card_id &&
      resolveClosingDayForDate
        ? resolveBillCompetence(singleExpenseDate, () =>
            resolveClosingDayForDate(singleExpenseDate),
          )
        : undefined);
    ...
    return [
      {
        ...
        ...(expense.report_weight !== undefined && {
          report_weight: expense.report_weight,
        }),
  ...
  return installmentAmounts.map((installmentAmount, index) => {
    ...
    if (expense.bill_competence && expense.payment_method === "credit_card") {
      ...
      billCompetence = resolveBillCompetence(installmentDate, () =>
        resolveClosingDayForDate(installmentDate),
      );
    }
