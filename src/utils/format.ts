...
export function getCurrentMonthString(): string {
  const d = new Date();
  return clampMonthToAppStart(
    `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`,
  );
}

/** Retorna a data de hoje no formato yyyy-MM-dd (ISO local) */
...
export function formatSignedPercentBR(
  value: number,
  fractionDigits = 2,
): string {
...
export function formatQuantityBR(
  value: number,
...
export function formatAxisCurrencyThousands(
  value: number,
