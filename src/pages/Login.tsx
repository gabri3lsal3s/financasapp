...
import { LogIn, Fingerprint, Lock, Mail, AlertCircle } from 'lucide-react';
...
export default function Login() {
  ...
  const handleBiometricLogin = async () => {
    ...
    try {
      ...
      // In Supabase, Passkey login uses the signInWithPasswordless or custom MFA challenge flow for WebAuthn
      // The experimental passkeys API is:
      // @ts-ignore - Supabase JS current types might not include signInWithWebAuthn yet on some versions, but the method exists in newer ones.
      ...
      setError(err.message || 'Falha ao autenticar com biometria. Você já a configurou nas Configurações?');
    } finally {
      setLoading(false);
    }
  };
        ...
        <Card className="mt-8">
          <form className="space-y-6" onSubmit={handleLogin}>
            ...
            <div className="space-y-3">
              ...
              >
                <Fingerprint className="h-5 w-5 text-accent transition-transform group-hover:scale-110" />
