...
export default function PortfolioTransactionFormModal({
  ...
  const [operationType, setOperationType] =
    useState<PortfolioOperationType>("buy");
  const [registrationType, setRegistrationType] = useState<"manual" | "b3">(
    "manual",
  );
  ...
  useEffect(() => {
    ...
    if (editingTransaction) {
      ...
      const isIncome = ["dividend", "jcp", "fii_yield"].includes(
        editingTransaction.operation_type,
      );
      ...
      if (isCash) {
        ...
        const totalVal =
          Number(editingTransaction.quantity) *
    ...
    const fetchCashAndDef = async () => {
      try {
        ...
        if (ticker) {
          ...
          if (defData) {
            const isB3 =
              defData.is_b3_linked || defData.pricing_mode === "market";
    ...
    try {
      ...
      if (isCashType || isIncomeType) {
        ...
        if (isNaN(unitPrice) || unitPrice <= 0) {
          throw new Error(
            isCashType
              ? "Insira um valor de caixa válido."
              : "Insira um valor de provento válido.",
          );
        }
      } else {
      ...
      if (!isCashType) {
        ...
        const defPayload = {
          ...
          manual_current_value:
            registrationType === "manual" && manualCurrentValue > 0
              ? manualCurrentValue
              : null,
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await cleanupOrphanPortfolioTickers(portfolioId, [
          ...
          <ModalFooter
            ...
            submitDisabled={
              ...
              (!(isCashType || isIncomeType) &&
                (!ticker.trim() || !price.trim()))
            }
            loading={saving}
            deleteLabel={isEditing ? "Excluir lançamento" : undefined}
            onDelete={
              isEditing ? () => setIsConfirmDeleteOpen(true) : undefined
            }
          />
        )}
      >
        <div className="space-y-4 text-left">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Select
              ...
              onChange={(e) => {
                ...
                const wasIncome = ["dividend", "jcp", "fii_yield"].includes(
                  operationType,
                );
                const isIncome = ["dividend", "jcp", "fii_yield"].includes(
                  newOp,
          ...
          {(() => {
            ...
            if (isCashType) {
              ...
              isInflow = ["sell", "dividend", "jcp", "fii_yield"].includes(
                operationType,
              );
            }
              ...
              <div className="p-3 bg-glass/5 border border-glass/20 rounded-xl animate-fade-in text-xs space-y-2 select-none">
                ...
                <div className="pt-2 border-t border-glass/15 flex items-center justify-between text-[11px]">
                  ...
                  <div className="flex items-center gap-1.5 font-mono">
                    <span className="text-secondary font-medium">
                      {formatCurrencyByCode(cashBalance, "BRL")}
                    </span>
        ...
        >
          <div className="space-y-2">
            <p className="text-sm text-secondary font-medium">
              Tem certeza de que deseja excluir permanentemente este lançamento
              do ativo{" "}
              <strong className="text-primary font-bold">{ticker}</strong>?
            </p>
            <p className="text-xs text-secondary/80">
              Esta ação removerá o lançamento da carteira e atualizará os saldos
              em caixa vinculados de forma automática.
            </p>
          </div>
        </ConfirmModal>
      )}
