...
import RecurringExpenseDetailModal from '@/components/dashboard/details/RecurringExpenseDetailModal'

/* ── Expense Row ── */
function ExpenseRow({
  item, onDismiss, onRestore, onClick, showDismiss = true,
}: {
  item: RecurringExpenseInfo
  onDismiss?: (desc: string) => void
  onRestore?: (desc: string) => void
  onClick?: (item: RecurringExpenseInfo) => void
  showDismiss?: boolean
}) {
  const levelConfig = {
  ...
  return (
    <button
      ...
      className={cn(
        'w-full flex items-center justify-between gap-1.5 sm:gap-2 px-2 sm:px-3 py-2 rounded-xl border border-glass transition-colors text-left',
        'hover:bg-secondary/10 hover:border-glass-strong',
    ...
    >
      <div className="flex items-center gap-1.5 sm:gap-2.5 min-w-0 flex-1">
        ...
        <div className="min-w-0">
          <p className="text-[10px] sm:text-xs font-bold text-primary truncate">{item.description}</p>
          <div className="flex items-center gap-1.5 sm:gap-2 mt-0.5 flex-wrap">
            <span className="text-[8px] sm:text-[9px] text-secondary/70 truncate max-w-[80px] sm:max-w-none">{item.categoryName}</span>
      ...
      <div className="flex items-center gap-1 sm:gap-2 shrink-0">
        ...
        {item.isIgnored && onRestore && (
          <span onClick={(e) => { e.stopPropagation(); onRestore(item.description) }} className="w-5 sm:w-6 h-5 sm:h-6 rounded-lg hover:bg-secondary/10 flex items-center justify-center text-secondary/40 hover:text-income shrink-0" title="Restaurar">
            <Eye size={10} />
          </span>
        )}
      </div>
...
export default function SubscriptionsDetail() {
  ...
  const [detailItem, setDetailItem] = useState<RecurringExpenseInfo | null>(null)
  ...
  return (
    <div className="space-y-2">
      ...
      <RecurringExpenseDetailModal
        ...
        item={detailItem ?? ({
          description: '', categoryName: '', monthlyAmount: 0, annualAmount: 0,
          monthsFound: 0, confidence: 0, recurrenceType: 'recurring', nature: 'variable',
          isIgnored: false, incomePercentage: 0, tier: 'discretionary',
          savingsIfCut: 0, tierReason: '', matchedExpenseIds: [], occurrencesByMonth: [],
          categoryId: undefined,
        } as RecurringExpenseInfo)}
      />
    </div>
  )
}
