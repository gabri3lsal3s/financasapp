...
import { roundToDecimals, todayISO } from "@/utils/format";
    ...
    try {
      if (editingExpense) {
        ...
        if (!error) {
          if (createLinkedDebt) {
            const categoryName =
              categories.find((c) => c.id === expenseData.category_id)?.name ||
              "Categoria";
            ...
            for (let i = 0; i < expensesToLink.length; i++) {
              ...
              const installmentSuffix =
                exp.installment_total && exp.installment_total > 1
                  ...
                  : "";
              const name =
                (expenseData.description || `Cobrança - ${categoryName}`) +
                installmentSuffix;
              await createDebt({
                name,
                type: "receivable",
                ...
                status: "pending",
                expense_id:
                  exp.id && !exp.id.startsWith("offline-") ? exp.id : null,
              });
            }
          }
          onClose();
  ...
  return (
    <>
      ...
      >
        ...
        <Select
          ...
          onChange={(e) =>
            setFormData((prev) => ({
              ...
              credit_card_id:
                e.target.value === "credit_card" ? prev.credit_card_id : "",
            }))
          }
          options={[
                ...
                ...creditCards
                  .filter(
                    (card) =>
                      ...
                      card.id === formData.credit_card_id,
                  )
              ...
              <div className="space-y-2">
                <Select
                  ...
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      bill_competence: e.target.value,
                    }))
                  }
                  options={[
        ...
        {editingExpense &&
          Number(editingExpense.installment_total || 1) > 1 && (
            <p className="modal-intro modal-panel-glass p-3">
              Esta despesa pertence ao parcelamento{" "}
              {editingExpense.installment_number || 1}/
              {editingExpense.installment_total}. A edição afeta apenas esta
              parcela.
            </p>
          )}
      </ModalForm>
