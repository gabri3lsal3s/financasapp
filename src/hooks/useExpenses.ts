...
import {
  ...
  generateInstallmentPayloads,
} from "@/utils/expenseInstallments";
...
import { APP_START_DATE, todayISO } from "@/utils/format";
...
export function useExpenses(month?: string) {
  ...
  const createExpense = async (
    ...
    try {
      ...
      if (expense.date && expense.date < APP_START_DATE) {
        throw new Error(
          `O app inicia em 01/01/2026. Lançamentos anteriores não são permitidos.`,
        );
      }
      ...
      let resolveClosingDayForDate:
        ((expenseDate: string) => number | undefined) | undefined;

      if (expense.payment_method === "credit_card" && expense.credit_card_id) {
        const startDate = expense.date || todayISO();
        const installmentDates = buildInstallmentDates(startDate, installments);
        const competences = installmentDates.map((dateValue) =>
          dateValue.substring(0, 7),
        );
        const closingDayContext = await fetchClosingDayForCard(
        ...
        resolveClosingDayForDate = (expenseDate: string) => {
          ...
          return (
            closingDayContext.closingDayByCompetence[competence] ||
            closingDayContext.defaultClosingDay
          );
        };
      }

      const expenseData = generateInstallmentPayloads(
        expense,
        installments,
        resolveClosingDayForDate,
      );

      const { data, error: insertError } = await supabase
        .from("expenses")
        .insert(expenseData).select(`
          *,
          category:categories(*),
          credit_card:credit_cards(*)
        `);
      ...
      if (shouldQueueOffline(err)) {
        ...
        const offlineIds = expenseData.map(
          (_, index) => `offline-${Date.now()}-${index}`,
        );
        ...
        try {
          ...
          logger.error(
            "Error loading metadata from cache during offline create:",
            e,
          );
        }
        ...
        const offlineExpenses: Expense[] = expenseData.map((payload, index) => {
          const installmentGroupId =
            "installment_group_id" in payload
              ? payload.installment_group_id
              : undefined;
          const installmentNumber =
            "installment_number" in payload
              ? payload.installment_number
              : undefined;
          const installmentTotal =
            "installment_total" in payload
          ...
          const matchedCard = payload.credit_card_id
            ? currentCards.find((c) => c.id === payload.credit_card_id)
            : undefined;
          ...
          return {
            ...
            report_weight:
              payload.report_weight !== undefined
                ? Number(payload.report_weight)
                : undefined,
            ...
            installment_number:
              installmentNumber !== undefined
                ...
                : null,
            installment_total:
              installmentTotal !== undefined ? Number(installmentTotal) : null,
            payment_method: payload.payment_method
              ? (String(payload.payment_method) as Expense["payment_method"])
              : undefined,
            credit_card_id: payload.credit_card_id
              ? String(payload.credit_card_id)
              : null,
            bill_competence: payload.bill_competence
              ? String(payload.bill_competence)
              : null,
            ...
            category: matchedCategory
              ? {
                  ...
                  color:
                    matchedCategory.color || "var(--category-fallback-muted)",
                }
              : undefined,
        ...
        window.dispatchEvent(
          new CustomEvent("local-data-changed", {
            detail: { entity: "expenses" },
          }),
        );
        return {
          data: offlineExpenses[0] || null,
          error: null,
          insertedExpenses: offlineExpenses,
        };
      }
  ...
  const updateExpense = async (id: string, updates: Partial<Expense>) => {
    ...
    try {
      ...
      const effectiveDate = String(
        updatePayload.date || existingExpense?.date || "",
      );

      if (effectiveDate && effectiveDate < APP_START_DATE) {
        throw new Error(
          `O app inicia em 01/01/2026. Alterações para datas anteriores não são permitidas.`,
        );
      }

      const effectivePaymentMethod = (updatePayload.payment_method ||
        existingExpense?.payment_method ||
        "other") as Expense["payment_method"];
      ...
      if (
        ...
        if (finalCompetence === undefined) {
          ...
          finalCompetence = resolveBillCompetence(
            effectiveDate,
            (c) =>
              closingDayContext.closingDayByCompetence[c] ||
              closingDayContext.defaultClosingDay,
          );
        }
      ...
      setExpenses((prev) =>
        sortExpensesByDate(prev.map((exp) => (exp.id === id ? data : exp))),
      );
      ...
      if (shouldQueueOffline(err)) {
        ...
        const nextState = sortExpensesByDate(
          expenses.map((exp) =>
            exp.id === id ? { ...exp, ...updatePayload } : exp,
          ),
        );
        setExpenses(nextState);
        await setCache(getCacheKey(), nextState);
  ...
  const deleteExpense = async (
    id: string,
    mode: "single" | "all" | "subsequent" = "single",
  ) => {
    ...
    try {
      ...
      if (mode !== "single" && targetGroupId) {
        idsToDelete = await fetchGroupExpenseIdsFromDb(
          targetGroupId,
          mode,
      ...
      const hasOfflineId = idsToDelete.some((idVal) =>
        idVal.startsWith("offline-"),
      );
      if (hasOfflineId) {
        throw new Error("Offline ID (bypass supabase)");
      }
      ...
      if (shouldQueueOffline(err)) {
        const offlineIdsToDelete = resolveExpenseIdsToDelete(
          expenses,
          id,
          mode,
        );
        ...
        const nextState = expenses.filter(
          (exp) => !offlineIdsToDelete.includes(exp.id),
        );
        setExpenses(nextState);
        await setCache(getCacheKey(), nextState);
        window.dispatchEvent(
