...
export function useIncomes(month?: string) {
  ...
  const createIncome = async (
    ...
    try {
      ...
      const incomeData = {
        ...
        ...(income.report_weight !== undefined && {
      ...
      if (shouldQueueOffline(err)) {
        ...
        try {
          currentIncomeCategories =
            (await getCache<IncomeCategory[]>("income_categories-all")) || [];
        } catch (e) {
          logger.error(
            "Error loading categories from cache during offline create:",
            e,
          );
        }
        const matchedCategory = currentIncomeCategories.find(
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Erro ao criar renda";
  ...
  const updateIncome = async (id: string, updates: Partial<Income>) => {
    try {
      ...
      setIncomes((prev) =>
        sortIncomesByDate(prev.map((inc) => (inc.id === id ? data : inc))),
      );
      return { data, error: null };
    } catch (err) {
      if (shouldQueueOffline(err)) {
        ...
        const nextState = sortIncomesByDate(
          incomes.map((inc) => (inc.id === id ? { ...inc, ...updates } : inc)),
        );
        setIncomes(nextState);
        await setCache(getCacheKey(), nextState);
        window.dispatchEvent(
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Erro ao atualizar renda";
      return { data: null, error: errorMessage };
    }
  };
  ...
  const deleteIncome = async (id: string) => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Erro ao deletar renda";
