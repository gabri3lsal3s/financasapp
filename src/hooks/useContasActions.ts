...
import { formatCurrency, roundToDecimals, todayISO } from "@/utils/format";
import type {
  CreditCard,
  Debt,
  Expense,
...
export interface UseContasActionsParams {
  ...
  createExpense: (
    data: Omit<Expense, "id" | "created_at" | "category" | "credit_card">,
  ...
  createIncome: (
    data: Omit<Income, "id" | "created_at" | "income_category">,
  ) => Promise<{ error: string | null }>;
  createDebt: (
    payload: Omit<Debt, "id" | "created_at">,
  ) => Promise<{ error: string | null }>;
  updateDebt: (
    id: string,
  ...
  createCreditCard: (
    payload: Omit<CreditCard, "id" | "created_at">,
  ) => Promise<{ error: string | null }>;
  updateCreditCard: (
    id: string,
...
export function useContasActions(params: UseContasActionsParams) {
  ...
  const getOrCreateRefundIncomeCategoryId = useCallback(async () => {
    const existing = incomeCategories.find((category) => {
      const normalizedName = String(category.name || "")
        ...
        .toLowerCase();
      return (
        normalizedName === REFUND_INCOME_CATEGORY_NAME.toLowerCase() ||
    ...
    if (existing?.id) {
      if (
        String(existing.name || "")
          .trim()
          .toLowerCase() === LEGACY_REFUND_INCOME_CATEGORY_NAME.toLowerCase()
      ) {
        await supabase
    ...
    if (error || !data?.id) {
      throw new Error(
        error?.message ||
          "Não foi possível criar a categoria de renda Estorno.",
      );
    }
  ...
  const handleConfirmDelete = useCallback(
    async (migrationCardId: string | null) => {
      ...
      try {
        ...
        const { error: deleteError } = await deleteCreditCard(
          modals.editingCard.id,
        );
        ...
        alert(
          `Erro ao excluir cartão: ${err instanceof Error ? err.message : "Ocorreu um erro inesperado"}`,
        );
      } finally {
  ...
  const handleOpenPaymentItem = useCallback(
    async (paymentItem: BillPaymentDisplayItem) => {
      ...
      if (parsedRefund.isRefund && parsedRefund.incomeId) {
        ...
        if (!data) {
          alert(
            "Este estorno não possui mais uma renda correspondente ativa no sistema.",
          );
          return;
        }
        ...
        modals.setEditingRefundIncomeInitialData({
          amount: data.amount,
          report_amount:
            data.report_weight !== undefined && data.report_weight !== null
  ...
  const handleSubmitEditRefundIncome = useCallback(
    async (payload: {
      amount: number;
      reportAmount: number;
      date: string;
      ...
      if (!modals.editingRefundPaymentItem || !modals.editingRefundIncomeId)
        return;

      const { amount, reportAmount, date, incomeCategoryId, description } =
        payload;
      const reportWeight =
      ...
      try {
        ...
        const refundNoteText = buildRefundNote(
          modals.editingRefundIncomeId,
          description || "",
        );
        ...
        alert(
          `Erro ao atualizar estorno: ${err instanceof Error ? err.message : "Erro desconhecido"}`,
        );
      }
    },
    [modals, loadBillData],
  ...
  const handleDeleteRefundIncome = useCallback(async () => {
    ...
    modals.setDeleteConfirmState({
      ...
      checkboxLabel:
        "Estou ciente de que este estorno será excluído permanentemente.",
      onConfirm: async () => {
        try {
          ...
          alert(
            `Erro ao excluir estorno: ${err instanceof Error ? err.message : "Erro desconhecido"}`,
          );
        }
      },
    });
  ...
  const handleSubmitRefund = useCallback(
    async (amount: number, date: string, description: string) => {
      ...
      try {
        ...
        if (!incomeData?.id)
          throw new Error(
            "Não foi possível obter o ID gerado para a receita de estorno.",
          );

        const refundNoteText = buildRefundNote(
          String(incomeData.id),
          description || "Estorno fatura",
        );
        ...
        alert(
          `Erro ao registrar estorno: ${err instanceof Error ? err.message : "Erro desconhecido"}`,
        );
      }
    },
  ...
  const handleSubmitCard = useCallback(
    async (payload: Omit<CreditCard, "id" | "created_at">) => {
      if (modals.editingCard) {
        const { error } = await updateCreditCard(
          modals.editingCard.id,
          payload,
        );
        if (error) {
  ...
  const handleSubmitEditPayment = useCallback(
    async (amount: number, date: string, note: string) => {
  ...
  const handleDeletePayment = useCallback(async () => {
    ...
    modals.setDeleteConfirmState({
      ...
      checkboxLabel:
        "Estou ciente de que este pagamento será excluído permanentemente.",
      onConfirm: async () => {
        const { error } = await supabase
          .from("credit_card_bill_payments")
          .delete()
  ...
  const handleSubmitCycle = useCallback(
    async (closingDay: number, dueDay: number) => {
  ...
  const handleSubmitEditExpense = useCallback(
    async (payload: {
      amount: number;
      reportAmount: number;
      date: string;
      ...
      const isRefund =
        Number(
          modals.editingExpenseItem.base_amount ??
            modals.editingExpenseItem.amount ??
            0,
        ) < 0;
      ...
      const reportWeight =
        payload.amount > 0
          ? roundToDecimals(payload.reportAmount / payload.amount, 4)
          : 1;
      ...
      const { error } = await updateExpense(modals.editingExpenseItem.id, {
        ...
        date: payload.date,
        payment_method:
          payload.paymentMethod as BillExpenseItem["payment_method"],
        credit_card_id:
          payload.paymentMethod === "credit_card" ? payload.creditCardId : null,
  ...
  const handleDeleteExpense = useCallback(async () => {
    ...
    if (Number(expenseItem.installment_total || 1) > 1) {
      ...
      modals.setDeleteConfirmState({
        ...
        checkboxLabel:
          "Estou ciente de que esta despesa será excluída permanentemente.",
        onConfirm: async () => {
  ...
  const handleSubmitDebt = useCallback(
    async (payload: {
      name: string;
  ...
  const handleDeleteDebt = useCallback(
    async (debtId: string) => {
      ...
      if (
        debt &&
        debt.expense?.installment_group_id &&
        Number(debt.expense?.installment_total || 1) > 1
      ) {
        modals.setDeleteModalState({
          isOpen: true,
          type: "debt",
        ...
        modals.setDeleteConfirmState({
          ...
          checkboxLabel:
            "Estou ciente de que esta pendência será excluída permanentemente.",
          onConfirm: async () => {
  ...
  const resolveIncomeCategoryId = useCallback(async () => {
    let cat = incomeCategories.find(
      (c) => (c.name || "").toLowerCase() === "outros",
    );
    if (cat) return cat.id;

    cat = incomeCategories.find(
      (c) => (c.name || "").toLowerCase() === "sem categoria",
    );
    if (cat) return cat.id;
    ...
    const { data: inserted, error: insertError } = await supabase
      .from("income_categories")
      .insert([
        { name: "Sem categoria", color: "var(--category-fallback-muted)" },
      ])
      .select("id")
      .single();
    ...
    if (insertError || !inserted?.id) {
      throw new Error(
        "Não foi possível obter ou criar categoria para a renda.",
      );
    }
    return inserted.id;
  }, [incomeCategories]);
  ...
  const handleConfirmWithIncome = useCallback(async () => {
    ...
    try {
      ...
      const { error: incomeError } = await createIncome({
        ...
        date: modals.selectedDebtForIncome.due_date || todayISO(),
      ...
      alert(
        `Erro ao criar receita: ${err instanceof Error ? err.message : "Erro desconhecido"}`,
      );
    }

    const { error } = await updateDebt(modals.selectedDebtForIncome.id, {
      status: "paid",
    });
    if (error) {
  ...
  const handleConfirmIntegrated = useCallback(async () => {
    ...
    if (
      ...
      finalValue < 0 ||
      finalValue > modals.linkedExpense.amount
    ) {
      alert(
        `Valor inválido. Deve ser entre 0 e ${formatCurrency(modals.linkedExpense.amount)}.`,
      );
      return;
    }
    ...
    try {
      const reportWeight =
        modals.linkedExpense.amount > 0
          ? roundToDecimals(finalValue / modals.linkedExpense.amount, 4)
          : 1;
      const { error: updateExpenseError } = await updateExpense(
        modals.linkedExpense.id,
        {
          report_weight: reportWeight,
        },
      );
      ...
      alert(
        `Erro ao processar integração da cobrança: ${err instanceof Error ? err.message : "Erro desconhecido"}`,
      );
    }
  ...
  const handleCreateExpenseForPayable = useCallback(
    async (expenseData: Omit<Expense, "id" | "created_at" | "category">) => {
      if (!modals.selectedDebtForPayableExpense)
        return { data: null, error: "Dívida não selecionada" };
      ...
      if (createdExpense) {
        const { error: updateDebtError } = await updateDebt(
          modals.selectedDebtForPayableExpense.id,
          {
            status: "paid",
        ...
        if (debt.type === "receivable") {
          if (debt.expense_id) {
            try {
              ...
              if (expense) {
                const currentReportValue = roundToDecimals(
                  expense.amount * (expense.report_weight ?? 1),
                  2,
                );
                const finalValue = Math.max(
                  0,
                  roundToDecimals(currentReportValue - debt.amount, 2),
                );
